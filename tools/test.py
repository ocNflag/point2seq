...
def parse_config():
    ...
    parser.add_argument('--cfg_file',
                        type=str,
                        default='/home/xuhang/xueyj/codebase/OpenPCDet/tools/cfgs/waymo_models/211025_e2e_seqfuse_bsf075.yaml',
    ...
    while True:
        ...
        if cur_epoch_id == -1 or int(float(cur_epoch_id)) < args.start_epoch:
            ...
            if total_time > args.max_waiting_mins * 60 and (first_eval is False):
                break
            if total_time > args.max_waiting_mins * 60 * 5:
                break
            continue
...
def main():
    ...
    args.start_epoch = cfg.OPTIMIZATION.NUM_EPOCHS - 1

    if args.runs_on == 'cloud':
        cfg.DATA_CONFIG.DATA_PATH = cfg.DATA_CONFIG.CLOUD_DATA_PATH
        cfg.MODEL.PRETRAINED_CKPT = cfg.MODEL.CLOUD_PRETRAINED_CKPT
    ...
    if args.runs_on == 'cloud':
        output_dir = Path(cfg.CLOUD_WORK_DIR)

    if args.extra_tag is None:
        args.extra_tag = Path(args.cfg_file).stem
    ...
    # if args.runs_on == 'cloud':
    #     cfg.MODEL.PRE_PATH = cfg.MODEL.CLOUD_PRE_PATH
