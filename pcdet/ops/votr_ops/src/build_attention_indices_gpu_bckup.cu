...
__global__ void sparse_strided_attention_with_tensor_kernel(int x_max, int y_max, int z_max, int x_stride, int y_stride, int z_stride,
    ...
    */
    ...
    for (int range_idx = 0; range_idx < num_range; ++range_idx) {
        ...
        for (int z_offset = search_z_start_range; z_offset < search_z_end_range; z_offset += search_z_stride) {
        for (int y_offset = search_y_start_range; y_offset < search_y_end_range; y_offset += search_y_stride) {
        for (int x_offset = search_x_start_range; x_offset < search_x_end_range; x_offset += search_x_stride) {
