...
__global__ void bev_sparse_strided_attention_with_hash_kernel(int x_max, int y_max, int z_max, int x_stride, int y_stride, int z_stride,
                                                            int num_voxels, int attend_size, int num_range, int hash_size,
                                                            int *attend_indices, const int *v_indices, const int *xyz_to_vidx, const int *range_spec) {
    /*
        attend_indices: [num_voxels, attend_size] for gather attend indices
    ...
    */
    ...
    for (int range_idx = 0; range_idx < num_range; ++range_idx) {
        ...
        int x_center_offset = x_stride / 2;
        int y_center_offset = y_stride / 2;
        int z_center_offset = z_stride / 2;

        for (int z_offset = 1; z_offset <= search_z_end_range; z_offset += search_z_stride) {
        for (int y_offset = 1; y_offset <= search_y_end_range; y_offset += search_y_stride) {
        for (int x_offset = 1; x_offset <= search_x_end_range; x_offset += search_x_stride) {
            if ((x_offset < search_x_start_range) && (y_offset < search_y_start_range)
             && (z_offset < search_z_start_range)) {
                continue;
             }
            ...
            z_step = 2 * z_offset - 1;
            for (
                int sz_idx = z_idx * z_stride + z_center_offset - z_offset;
                sz_idx <= z_idx * z_stride + z_center_offset + z_offset - 1;
                sz_idx += z_step){

                if (sz_idx >= z_max || sz_idx < 0) continue;
                y_step = 2 * y_offset - 1;
                for (
                    int sy_idx = y_idx * y_stride + y_center_offset - y_offset;
                    sy_idx <= y_idx * y_stride + y_center_offset + y_offset - 1;
                    sy_idx += y_step){

                    if (sy_idx >= y_max || sy_idx < 0) continue;
                    x_step = 2 * x_offset - 1;
                    for (
                        int sx_idx = x_idx * x_stride + x_center_offset - x_offset;
                        sx_idx <= x_idx * x_stride + x_center_offset + x_offset - 1;
                        sx_idx += x_step){
                        if (sx_idx >= x_max || sx_idx < 0) continue;
                        int skey = sx_idx * y_max * z_max + sy_idx * z_max + sz_idx;
                        int sv_idx = hash_table_find(skey, hash_size, xyz_to_vidx);
...
void bev_sparse_strided_attention_with_hash_kernel_launcher(int x_max, int y_max, int z_max, int x_stride, int y_stride, int z_stride,
                                                            int num_voxels, int attend_size, int num_range, int hash_size,
                                                            int *attend_indices, const int *v_indices, const int *xyz_to_vidx, const int *range_spec){
    ...
    bev_sparse_strided_attention_with_hash_kernel<<<blocks, threads>>>(x_max, y_max, z_max, x_stride, y_stride, z_stride,
                                                                        num_voxels, attend_size, num_range, hash_size,
                                                                        attend_indices, v_indices, xyz_to_vidx, range_spec);
    // cudaDeviceSynchronize();  // for using printf in kernel function
    err = cudaGetLastError();
