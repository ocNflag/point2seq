...
__device__ int hash(int k, int max_hash_size) {
    k ^= k >> 16;
    k *= 0x85ebca6b;
    k ^= k >> 13;
    k *= 0xc2b2ae35;
...
__global__ void fill_dense_hash_kernel(int x_max, int y_max, int z_max, int num_total_voxels, int max_hash_size, 
    ...
    */
    ...
    int hash_idx = hash(key, max_hash_size);
    int prob_cnt = 0;
    while(true) {
...
void fill_dense_hash_kernel_launcher(int x_max, int y_max, int z_max, int num_total_voxels,
    ...
    fill_dense_hash_kernel<<<blocks, threads>>>(x_max, y_max, z_max, num_total_voxels,
