...
int point_to_voxel_query_bin_wrapper( float x_size, float y_size, float z_size, int x_max, int y_max, int z_max,
    ...
    int *v_bin = v_bin_tensor.data<int>();

    point_to_voxel_query_bin_kernel_launcher(x_size, y_size, z_size, x_max, y_max, z_max,
                                                ...
                                                p_bs_cnt, v_bs_cnt, xyz, xyz_to_vidx,
                                                v_mask, v_bin);
    return 1;
}
...
int merge_pv_bin_wrapper(int x_divide, int y_divide, int z_divide, int num_total_voxels, int num_samples, int batch_size,
    ...
    const int *v_bs_cnt = v_bs_cnt_tensor.data<int>();
    const int *v_bin = v_bin_tensor.data<int>();
    int *v_map = v_map_tensor.data<int>();

    merge_pv_bin_kernel_launcher(x_divide, y_divide, z_divide, num_total_voxels, num_samples, batch_size,
                                    v_bs_cnt, v_bin, v_map);
    return 1;
}