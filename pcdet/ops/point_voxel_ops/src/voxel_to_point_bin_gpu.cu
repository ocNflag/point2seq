...
__device__ int hash_b(int k, int max_hash_size) {
    k ^= k >> 16;
    k *= 0x85ebca6b;
    k ^= k >> 13;
    k *= 0xc2b2ae35;
...
__global__ void voxel_to_point_query_bin_kernel(float x_size, float y_size, float z_size, int x_max, int y_max, int z_max,
    ...
    for (int z_idx = z_min_idx; z_idx < z_max_idx; z_idx += z_dilate) {
        ...
        for (int y_idx = y_min_idx; y_idx < y_max_idx; y_idx += y_dilate) {
            ...
            for (int x_idx = x_min_idx; x_idx < x_max_idx; x_idx += x_dilate) {
                ...
                int hash_idx = hash_b(key, max_hash_size);
                int v_idx = -1;
                int prob_cnt = 0;
                while (true) {
                    // found
...
void voxel_to_point_query_bin_kernel_launcher(float x_size, float y_size, float z_size, int x_max, int y_max, int z_max,
                                                int x_step, int y_step, int z_step, int x_dilate, int y_dilate, int z_dilate,
                                                int x_divide, int y_divide, int z_divide,
                                                int num_total_points, int num_total_voxels, int num_samples, int max_hash_size,
                                                const int *p_bs_cnt, const float *xyz, const int *xyz_to_vidx,
    ...
    voxel_to_point_query_bin_kernel<<<blocks, threads>>>(x_size, y_size, z_size, x_max, y_max, z_max,
