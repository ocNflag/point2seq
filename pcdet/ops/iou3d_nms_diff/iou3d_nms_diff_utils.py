...
from . import iou3d_nms_diff_cuda, numerical_jaccobian
import numba
...
def boxes_bev_iou_cpu(boxes_a, boxes_b):
    ...
    iou3d_nms_diff_cuda.boxes_iou_bev_cpu(boxes_a.contiguous(), boxes_b.contiguous(), ans_iou)
...
def boxes_iou_bev(boxes_a, boxes_b):
    ...
    iou3d_nms_diff_cuda.boxes_iou_bev_gpu(boxes_a.contiguous(), boxes_b.contiguous(), ans_iou)
...
def boxes_iou3d_gpu(boxes_a, boxes_b):
    ...
    iou3d_nms_diff_cuda.boxes_overlap_bev_gpu(boxes_a.contiguous(), boxes_b.contiguous(), overlaps_bev)
...
def nms_gpu(boxes, scores, thresh, pre_maxsize=None, **kwargs):
    ...
    num_out = iou3d_nms_diff_cuda.nms_gpu(boxes, keep, thresh)
...
def nms_normal_gpu(boxes, scores, thresh, **kwargs):
    ...
    keep = torch.LongTensor(boxes.size(0))
    num_out = iou3d_nms_diff_cuda.nms_normal_gpu(boxes, keep, thresh)
    return order[keep[:num_out].cuda()].contiguous(), None
...
class BoxesIou3dDifferentiableFunction(torch.autograd.Function):
    ...
    def call_func(input):
        ...
        iou3d_nms_diff_cuda.boxes_overlap_bev_onebyone_gpu(boxes_a.contiguous(), boxes_b.contiguous(), overlaps_bev)
