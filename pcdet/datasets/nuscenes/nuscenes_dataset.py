...
class NuScenesDataset(DatasetTemplate):
    ...
    def double_flip(self, input_dict):
        ...
        yflip_points[:, 1] = -points[:, 1]
        xflip_points = points.copy()
        xflip_points[:, 0] = -points[:, 0]
        dflip_points = points.copy()
        dflip_points[:, 0] = -points[:, 0]
        dflip_points[:, 1] = -points[:, 1]
        xflip_dict, yflip_dict, dflip_dict = {}, {}, {}
        for key, value in input_dict.items():
            if key == 'points':
                yflip_dict[key] = yflip_points
                xflip_dict[key] = xflip_points
                dflip_dict[key] = dflip_points
            else:
    ...
    def __getitem__(self, index):
        ...
        if not self.training and self.dataset_cfg.get('USE_DOUBLE_FLIP_TEST', False):
            ...
            yflip_data_dict = self.prepare_data(data_dict=yflip_dict)
            xflip_data_dict = self.prepare_data(data_dict=xflip_dict)
            dflip_data_dict = self.prepare_data(data_dict=dflip_dict)
        ...
        if not self.training and self.dataset_cfg.get('USE_DOUBLE_FLIP_TEST', False):
            return (data_dict, yflip_data_dict, xflip_data_dict, dflip_data_dict)
        else:
            return data_dict
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--cfg_file',
        type=str,
        default='/home/xuhang/xueyj/codebase/OpenPCDet/tools/cfgs/dataset_configs/nuscenes_dataset.yaml',
    ...
    if args.func == 'create_nuscenes_infos':
        ...
        if args.runs_on == 'server':
            ...
            raise Exception('runs_on options have to be server or cloud')
        # tasks = dataset_cfg.TRACKING_TASKS
