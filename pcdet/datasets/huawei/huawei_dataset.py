...
class HuaweiDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.huawei_infos = []
        self.include_huawei_data(self.split)

    def include_huawei_data(self, split):
        if self.logger is not None:
            self.logger.info('Loading Huawei dataset')
        huawei_infos = []
        ...
        for info_path in self.dataset_cfg.INFO_PATH[split]:
            ...
            with open(info_path, 'rb') as f:
                infos = pickle.load(f)
                huawei_infos.extend(infos)

        def check_annos(info):
            return 'annos' in info

        if self.split != 'raw':
            huawei_infos = list(filter(check_annos,huawei_infos))

        self.huawei_infos.extend(huawei_infos)

        if self.logger is not None:
            self.logger.info('Total samples for Huawei dataset: %d' % (len(huawei_infos)))
    ...
    def point_painting(self, points, info):
        semseg_dir = '/home/xuhang/maojiageng/HRNet/data/seg/segmentation_new_outputs'
    ...
    def __len__(self):
        if self._merge_all_iters_to_one_epoch:
            return len(self.huawei_infos) * self.total_epochs

        return len(self.huawei_infos)

    def __getitem__(self, index):
        if self._merge_all_iters_to_one_epoch:
            index = index % len(self.huawei_infos)

        info = copy.deepcopy(self.huawei_infos[index])
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        db_info_save_path = Path(self.root_path) / ('huawei_dbinfos_%s.pkl' % split)
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        from .huawei_eval.evaluation import get_evaluation_results

        eval_det_annos = copy.deepcopy(det_annos)
        eval_gt_annos = [copy.deepcopy(info['annos']) for info in self.huawei_infos]
...
def create_huawei_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = HuaweiDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)

    splits = ['train', 'val', 'test', 'raw_small', 'raw_medium', 'raw_large']
    ignore = ['train', 'val', 'raw_small', 'raw_medium', 'raw_large']
    ...
    for split in splits:
        ...
        filename = 'huawei_infos_%s.pkl' % split
        filename = save_path / Path(filename)
        dataset.set_split(split)
        huawei_infos = dataset.get_infos(num_workers=workers)
        with open(filename, 'wb') as f:
            pickle.dump(huawei_infos, f)
        print('Huawei info %s file is saved to %s' % (split, filename))

    #"""
    train_filename = save_path / 'huawei_infos_train.pkl'
...
if __name__ == '__main__':
    ...
    if args.func == 'create_huawei_infos':
        import yaml
        from pathlib import Path
        ...
        huawei_data_path = ROOT_DIR / 'data' / 'huawei'
        huawei_save_path = ROOT_DIR / 'data' / 'huawei'

        if args.runs_on == 'cloud':
            huawei_data_path = Path('/cache/huawei/')
            huawei_save_path = Path('/cache/huawei/')
            dataset_cfg.DATA_PATH = dataset_cfg.CLOUD_DATA_PATH

        create_huawei_infos(
            dataset_cfg=dataset_cfg,
            class_names=['Car', 'Bus', 'Truck', 'Pedestrian', 'Bicycle'],
            data_path=huawei_data_path,
            save_path=huawei_save_path
        )