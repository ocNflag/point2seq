...
def split_huawei_semi_data(info_paths, data_splits, root_path, labeled_ratio, logger):
    ...
    for info_path in info_paths[train_split]:
        ...
        with open(info_path, 'rb') as f:
            ...
            huawei_pretrain_infos.extend(copy.deepcopy(infos))
            huawei_labeled_infos.extend(copy.deepcopy(infos))
    ...
    for info_path in info_paths[test_split]:
        ...
        with open(info_path, 'rb') as f:
            infos = pickle.load(f)
            infos = list(filter(check_annos, infos))
            huawei_test_infos.extend(copy.deepcopy(infos))
    ...
    for info_path in info_paths[raw_split]:
        ...
        with open(info_path, 'rb') as f:
            infos = pickle.load(f)
            huawei_unlabeled_infos.extend(copy.deepcopy(infos))

    logger.info('Total samples for Huawei pre-training dataset: %d' % (len(huawei_pretrain_infos)))
    logger.info('Total samples for Huawei testing dataset: %d' % (len(huawei_test_infos)))
    logger.info('Total samples for Huawei labeled dataset: %d' % (len(huawei_labeled_infos)))
    logger.info('Total samples for Huawei unlabeled dataset: %d' % (len(huawei_unlabeled_infos)))

    return huawei_pretrain_infos, huawei_test_infos, huawei_labeled_infos, huawei_unlabeled_infos

class HuaweiSemiDataset(SemiDatasetTemplate):
...
class HuaweiPretrainDataset(HuaweiSemiDataset):
    def __init__(self, dataset_cfg, class_names, infos=None, training=True, root_path=None, logger=None):
        """
        Args:
            root_path:
...
class HuaweiLabeledDataset(HuaweiSemiDataset):
    def __init__(self, dataset_cfg, class_names, infos=None, training=True, root_path=None, logger=None):
        """
        Args:
            root_path:
...
class HuaweiUnlabeledDataset(HuaweiSemiDataset):
    def __init__(self, dataset_cfg, class_names, infos=None, training=True, root_path=None, logger=None):
        """
        Args:
            root_path:
...
class HuaweiTestDataset(HuaweiSemiDataset):
