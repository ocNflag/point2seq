...
class CenterHead(nn.Module):
    ...
    def get_loss(self):
        ...
        for task_id, pred_dict in enumerate(pred_dicts):
            ...
            tmp_list = [pred_dict['reg'], pred_dict['height'],
                        pred_dict['dim'], pred_dict['rot']]
            if self.dataset == 'nuscenes':
                tmp_list.append(pred_dict['vel'])
            pred_box_encoding = torch.cat(tmp_list, dim = 1).contiguous() # (B, 10, H, W)
    ...
    def proposal_layer(self, heat, rots, rotc, hei, dim, vel, reg = None,
        ...
        tmp_list = [xs, ys, hei, dim, rot]
        if self.dataset == 'nuscenes':
            vel = self._transpose_and_gather_feat(vel, inds)
            vel = vel.view(batch, K, 2)
            tmp_list.append(vel)
        # vel after rot
        final_box_preds = torch.cat(tmp_list, dim=2)
        final_scores = scores
        final_preds = clses
