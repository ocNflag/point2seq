...
import numpy as np
import numba
from .attention_fusion_module import AttnDCNFusionHead
...
class AttnFusionHead(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, voxel_size, point_cloud_range,
        ...
        self.feat_ch = model_cfg.PARAMETERS.feat_ch
        self.key_ch = model_cfg.PARAMETERS.key_ch
        self.value_ch = model_cfg.PARAMETERS.value_ch
        ...
        for num_cls in self.num_classes:
            ...
            self.tasks.append(
                AttnDCNFusionHead(shared_ch, num_cls, heads, feat_ch=self.feat_ch, key_ch=self.key_ch,
                                  value_ch=self.value_ch, init_bias=self.init_bias, final_kernel=3)
            )
