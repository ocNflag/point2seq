...
import numpy as np
import numba
from pcdet.models.dense_heads.utils import NaiveSepHead, _sigmoid
from pcdet.models.dense_heads.bundle_modules import DCNBundleHead
...
class BundleHead(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, voxel_size, point_cloud_range,
        ...
        for num_cls in self.num_classes:
            ...
            if not self.use_dcn:
                heads.update(dict(center_map=(num_cls, 2), corner_map=(4 * num_cls, 2), foreground_map=(num_cls, 2)))
                self.tasks.append(
                    NaiveSepHead(share_conv_channel, heads, bn=True, init_bias=self.init_bias, final_kernel=3,
                                 directional_classifier=False)
                )
            else:
                self.tasks.append(
                    DCNBundleHead(share_conv_channel, num_cls, heads, bn=True, init_bias=self.init_bias, final_kernel=3,
                                  directional_classifier=False)
                )
    ...
    def get_loss(self):
        ...
        for task_id, pred_dict in enumerate(pred_dicts):
            ...
            # TODO modify it from aux loss to fuse model
    ...
    def _double_flip_process(self, pred_dict, batch_size):
        ...
        batch_center = pred_dict['center_map'].sigmoid()
    ...
    def generate_predicted_boxes(self, data_dict):
        ...
        for task_id, pred_dict in enumerate(pred_dicts):
            ...
            if double_flip:
                assert batch_size % 4 == 0, print('cannot be divided by 4, batch_size=' + str(batch_size))
