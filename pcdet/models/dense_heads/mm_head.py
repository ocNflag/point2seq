...
from .target_assigner.mm_assigner import MMAssigner
...
class MMHead(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, voxel_size, point_cloud_range,
        ...
        self.target_assigner = MMAssigner(
    ...
    def proposal_layer(self, heat, rots, rotc, hei, dim, vel, reg = None,
        ...
        cls_selected = clses.reshape(-1).long() # (batch * K)
        ...
        # decode height
        z = self.model_cfg.TASKS[task_id].z
        z = torch.tensor(z).to(hei.device)
        z = z[cls_selected] # (batch * K, 1)
        hei = hei + z
        hei = hei.reshape(batch, K, 1)

        # dim of the box
        dim = self._transpose_and_gather_feat(dim, inds)
        dim = dim.view(batch * K, 3)
        # decode dim
        anchors = self.model_cfg.TASKS[task_id].anchors
        anchors = torch.tensor(anchors).to(dim.device)
        anchors = anchors[cls_selected] # (batch * K, 3)
        dim = torch.exp(dim) * anchors
        dim = dim.reshape(batch, K, 3)
