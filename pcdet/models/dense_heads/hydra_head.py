...
import numpy as np
import numba
from pcdet.models.dense_heads.utils import NaiveSepHead, DCNFusionHead, NaiveFusionHead, _sigmoid
from pcdet.models.dense_heads.hydra_modules import HydraFusionHead
...
class HydraHead(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, voxel_size, point_cloud_range,
        ...
        self.task_sampler_params = [t["task_sampler_params"] for t in model_cfg.TASKS]
        ...
        self.shared_conv = nn.Sequential(
            nn.Conv2d(self.in_ch, shared_ch,
                      kernel_size=3, padding=1, bias=True),
            nn.BatchNorm2d(shared_ch),
            nn.ReLU(inplace=True)
        )

        self.task_head_params = model_cfg.PARAMETERS.task_head_params
        self.task_head_params.update({
        ...
        for num_cls, sampler_params in zip(self.num_classes, self.task_sampler_params):
            heads = copy.deepcopy(self.common_heads)
            heads.update(dict(fusion_map=[num_cls, 2]))

            task_head_params = copy.deepcopy(self.task_head_params)
            task_head_params['sampler_params'] = sampler_params


            self.tasks.append(
                HydraFusionHead(num_cls, heads, **task_head_params)
            )
    ...
    def forward(self, data_dict):
        ...
        print('before shared conv', torch.isnan(spatial_features_2d).any())
        spatial_features_2d = self.shared_conv(spatial_features_2d)
        print('after shared conv', torch.isnan(spatial_features_2d).any())
