...
class E2EROIHead(nn.Module):
    def __init__(self, input_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        self.pib_layer = PointInBoxTransformer(feat_ch=2, emb_ch=64, out_ch=64, num_heads=2)
        self.rib_layer = SelfAttnTransformer(in_ch=64, emb_ch=64, out_ch=64, num_heads=2)

        box_ch = 7
        if self.use_roi_score:
            box_ch += num_class

        self.box_layer = BoxLevelTransformer(box_ch=box_ch, in_ch=64, emb_ch=64, out_ch=64, num_heads=2)
