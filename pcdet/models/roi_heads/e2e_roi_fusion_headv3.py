...
class SelfAttnBlock(nn.Module):
    def __init__(self, in_ch, emb_ch, out_ch, num_heads=2, layer=3):
        super().__init__()
...
class E2EROIFusionHeadV3(nn.Module):
    def __init__(self, input_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        self.rib_layers = nn.ModuleDict()

        if self.use_point:
            self.pib_layers['points'] = PointInBoxBlock(
                feat_ch=self.in_ch_map['points'],
                emb_ch=self.emb_ch_map['points'],
                out_ch=self.emb_ch_map['points'],
                num_heads=2,
                layer=self.pib_layer_num
            )
            self.rib_layers['points'] = SelfAttnBlock(
                in_ch=self.emb_ch_map['points'],
                emb_ch=64,
                out_ch=64,
                num_heads=2,
        ...
        for src_name in self.voxel_src:
            ...
            self.rib_layers[src_name] = SelfAttnBlock(
                in_ch=self.emb_ch_map[src_name],
                emb_ch=64,
                out_ch=64,
                num_heads=2,
                layer=self.rib_layer_num
        ...
        self.box_layer = BoxLevelTransformer(box_ch=9, in_ch=64 * len(self.input_feats), emb_ch=256, out_ch=256, num_heads=2)
    ...
    def get_voxel_infos(self, batch_dict):
        ...
        for src_name in self.voxel_src:
            ...
            features = self.adapt_layers[src_name](features)
            batch_id = cur_coords[:, 0].int().contiguous()
    ...
    def get_bev_infos(self, batch_dict):
        ...
        bs, m, _ = rois.shape
        if self.use_fusion:
            spatial_features = self.adapt_layers['bev'](batch_dict['final_feat'])
        else:
            spatial_features = self.adapt_layers['bev'](batch_dict['spatial_features_2d'])

        roi_center = rois[:, :, 0:3].view(bs, m, 1, 3).contiguous()
        roi_corners = box_utils.boxes_to_sps_3d(rois.view(bs * m, -1)[:, :7].contiguous()).view(bs, m, self.sample_points - 1, 3)
        roi_queries = torch.cat([roi_corners, roi_center], dim=2)[..., :2].contiguous().view(bs, m * self.sample_points, 2)
        ...
        return bev_features.view(bs * m, self.sample_points, -1).permute(1, 0, 2).contiguous()
    ...
    def forward(self, batch_dict):
        ...
        if self.use_point:
            tmp_feat = self.pib_layers['points'](
                ref_points,
            ...
            ref_feat_dict['points'] = self.rib_layers['points'](tmp_feat, ref_points).view(b, nr, -1).permute(1, 0, 2).contiguous()

        if self.use_bev:
            tmp_feat = self.get_bev_infos(batch_dict)
            ref_feat_dict['bev'] = self.rib_layers['bev'](tmp_feat, ref_points).view(b, nr, -1).permute(1, 0, 2).contiguous()

        voxel_dict = self.get_voxel_infos(batch_dict)
        for src_name in self.voxel_src:
            tmp = voxel_dict[src_name]
            np = tmp['coords'].shape[2]
            tmp_feat = self.pib_layers[src_name](
                ref_points,
            ...
            ref_feat_dict[src_name] = self.rib_layers[src_name](tmp_feat, ref_points).view(b, nr, -1).permute(1, 0, 2).contiguous()


        ref_feat_list = [ref_feat_dict[i] for i in ref_feat_dict]
        box_feats = torch.cat(ref_feat_list, dim=-1)
