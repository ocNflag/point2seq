...
class E2EROIHeadV3(nn.Module):
    def __init__(self, input_channels, model_cfg, num_class=1, **kwargs):
        ...
        self.grid_size = self.model_cfg.get('GRID_SIZE', 6)
        self.sample_points = self.grid_size ** 3
    ...
    def get_roi_ref_points(self, batch_dict):
        ...
        bs, m, _ = rois.shape
        sample_points = self.get_dense_grid_points(rois.view(bs * m, -1)[:, :7].contiguous()).view(bs, m, self.sample_points, 3)
        return sample_points

    def get_dense_grid_points(self, rois):
        batch_size_rcnn = rois.shape[0]
        faked_features = rois.new_ones((self.grid_size, self.grid_size, self.grid_size))
        dense_idx = faked_features.nonzero()  # (N, 3) [x_idx, y_idx, z_idx]
        dense_idx = dense_idx.repeat(batch_size_rcnn, 1, 1).float()  # (B, 6x6x6, 3)

        local_roi_size = rois[:, 3:6]
        roi_grid_points = (dense_idx + 0.5) / self.grid_size * local_roi_size.unsqueeze(dim=1) \
